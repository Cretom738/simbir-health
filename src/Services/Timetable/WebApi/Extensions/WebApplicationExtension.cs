...
namespace WebApi.Extensions
{
    public static class WebApplicationExtension
    {
        ...
        public static void AddRabbitmq(this WebApplicationBuilder builder)
        {
            ...
            {
                c.AddConsumer<AccountDeletedConsumer>();

                c.AddConsumer<HospitalDeletedConsumer>();

                c.AddConsumer<HospitalUpdatedConsumer>();
                ...
                c.UsingRabbitMq((context, cfg) =>
                {
                    ...
                    cfg.ReceiveEndpoint("account-deleted", c =>
                    {
                        c.ConfigureConsumer<AccountDeletedConsumer>(context);
                    });

                    cfg.ReceiveEndpoint("hospital-deleted", c =>
                    {
                        c.ConfigureConsumer<HospitalDeletedConsumer>(context);
                    });

                    cfg.ReceiveEndpoint("hospital-updated", c =>
                    {
                        c.ConfigureConsumer<HospitalUpdatedConsumer>(context);
                    });
                });
            });
        }
